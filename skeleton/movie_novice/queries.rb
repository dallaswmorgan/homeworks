...
def pulp_fiction_actors
  ...
  Actor.select('actors.*').joins(:castings).where("castings.movie_id = (
    SELECT id
    FROM movies
...
def uma_movies
  ...
  Movie.select('movies.id, movies.title, movies.yr').joins(:castings)
  .where("castings.actor_id = (SELECT id FROM actors WHERE name = 'Uma Thurman')")
end
